/*
  Level: 5 kyu
  Link: https://www.codewars.com/kata/52336a4436e0b095d8000093
  Instructions: 
    ...
    (1. That every list of 1 element is "sorted" ... and 2. It is much easier to combine two already-sorted lists 
    into 1 sorted list than it is to sort a big list all at once) to yield an algorithm with a worst-case complexity of O(n log n).
    ...
    The name of the function in this example is "mergesorted". It should return a big sorted array from two smaller 
    sorted arrays: e.g.
...
function mergesorted(a, b) {
  ...
  while (left + right < a.length + b.length) {     
    if (right === b.length || b[right] > a[left]) {
      result.push(a[left]);
      left++;
    } else {
      result.push(b[right]);
