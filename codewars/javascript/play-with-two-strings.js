/*
  ...
  Instructions: 
    ...
    everything is ok without bad tricks, only two input strings and as result 
    one output string;-)...

    And here's the rule:
    Input Strings a and b: For every character in string a swap the casing of every 
    occurrence of the same character in string b. Then do the same casing swap with 
    the inputs reversed. Return a single string consisting of the changed version of a 
    followed by the changed version of b. A char of a is in b regardless if it's in upper 
    or lower case - see the testcases too.
    I think that's all;-)...
...
function swapCase(str, map) {
  let result = '';
  for (let char of str) {
    let lowerChar = char.toLowerCase();
    if (map[lowerChar] && map[lowerChar] % 2 !== 0) {
      char = isUpper(char) ? lowerChar : char.toUpperCase();
    }
    result += char;
  }
  return result
...
function workOnStrings(a, b) {
  const aMap = getCharMap(a);
  const bMap = getCharMap(b);
  return swapCase(a, bMap) + swapCase(b, aMap);
}