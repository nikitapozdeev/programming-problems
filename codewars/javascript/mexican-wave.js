/*
  ...
  Instructions: 
    ...
    the crowd is seated in a contiguous circuit all the way around the sport field, and 
    so the wave is able to travel continuously around the arena; in discontiguous seating 
    arrangements, the wave can instead reflect back and forth through the crowd. When the 
    gap in seating is narrow, the wave can sometimes pass through it. Usually only one 
    wave crest will be present at any given time in an arena, although simultaneous, 
...
function wave(str){
  return [...str].reduce((acc, curr, index) => {
    if (curr !== ' ') {
      ...
      const suffix = str.substr(index + 1);
      acc.push(`${prefix}${upper}${suffix}`);
    }
    return acc;
  }, []);
}