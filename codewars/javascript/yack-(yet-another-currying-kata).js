/*
  Level: 4 kyu
  Link: https://www.codewars.com/kata/55f91566e0d2317066000057
  Instructions: 
    ...
    curriedAdd(1)(2)(3); //6
    Your job is to implement the yack function which allows currying any function passed 
    as an argument.
    ...
    curriedAdd(2)(3); //6
    curriedAdd(2)(4); //7
    In the implementation of the solution, you should not use any of the following 
    functions: (Note: Actually the tests are not checking that these methods are not used 
    because I do not know how to do it. Any help is welcome).
...
function yack(fn, ...fnArgs) {
  if (fnArgs.length >= fn.length) {
    return fn(...fnArgs);
  }
  
  return (...args) => yack(fn, ...fnArgs.concat(args));
}