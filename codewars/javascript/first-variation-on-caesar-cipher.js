/*
  ...
  Instructions: 
    The action of a Caesar cipher is to replace each plaintext letter (plaintext letters 
    are from 'a' to 'z' or from 'A' to 'Z') with a different one a fixed number of places 
    ...
    strings containing the coded string in 5 parts (five parts because, to avoid more 
    ...
    of 4, 4, 4, 4, 1. The parts 1, 2, 3, 4 are evenly split and the last part of length 
    1 is shorter. If the length is 16 the parts will be of lengths 4, 4, 4, 4, 0. Parts 
    1, 2, 3, 4 are evenly split and the fifth runner will stay at home since his part is 
    the empty string. If the length is 11, equal parts would be of length 2.2, hence parts 
...
function wrap(lower, upper, value) {
  ...
  if (value < lower) {
    const shift = lower - value - 1;
    const offset = (lower - value) < size 
      ? shift
      : shift % size
    return upper - offset;
  } else {
...
function encode(str, shift, forward) {
  let result = '';
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    let offset = direction * (shift + i);
    let shiftedChar = charCode + offset;
    
    if (charCode >= 65 && charCode <= 90) {
      charCode = wrap(65, 90, shiftedChar);
    } else if (charCode >= 97 && charCode <= 122) {
      charCode = wrap(97, 122, shiftedChar);
    } 
...
function movingShift(s, shift) {
  const encodedStr = encode(s, shift, true);
  const chunks = [];
  const chunkSize = Math.ceil(encodedStr.length / 5);
  
  for (let i = 0; i < 5; i++) {
    chunks.push(encodedStr.substr(i * chunkSize, chunkSize));
  }
  return chunks;
}

function demovingShift(arr, shift) {
  const encodedStr = arr.join('');
  const decodedStr = encode(encodedStr, shift, false);
  return decodedStr;
}